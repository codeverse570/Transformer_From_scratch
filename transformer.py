...
class Transformer :
    ...
    def fit(self,encoder_inputs,decoder_inputs,targets,enc_k,enc_q,dec_k,dec_q):
        ...
        #  print(self.emb.weight.mean(),self.emb.weight.std())
...
if __name__ == "__main__":
    ...
    x_validation_encoder= torch.as_tensor(x_validation_encoder, device=device)
    x_validation_decoder= torch.as_tensor(x_validation_decoder,device=device)
    x_validation_target= torch.as_tensor(x_validation_target,device=device).long()
